...
"""
=============================================================================
CONTROLADOR ESPECIALIZADO: MIAMI LMD (v1.0 RADIOGRAFÍA & AUTO-LOGIN)
=============================================================================
Flujo:
  1. Recarga en vivo (Live Reload) de datos frescos del Consulado de Miami.
  2. Puerta Cloudflare Turnstile con trayectoria humana.
  3. Botón verde [ Continue / Continuar ].
  4. Pantalla de Calendario (#datetime):
     - Apertura física del Datepicker (#idDivBktDatetimeDatePickerContent).
     - Extracción y agrupación jerárquica de TODOS los días habilitados por MESES.
     - Extracción de franjas horarias.
  5. Inyección asistida de credenciales en #signupsecondappointment (#idIptBktSignInlogin, #idIptBktSignInpassword).
=============================================================================
"""
...
URL_MIAMI_LMD = "https://www.citaconsular.es/es/hosteds/widgetdefault/2533f04b1d3e818b66f175afc9c24cf63"
...
def verificar_citas_miami(page, config, log_fn=print):
    """
    Función principal de escaneo para Miami LMD.
    Retorna: (hay_citas: bool, resumen_general: str, info_slots: dict)
    """
    log_fn("\n[RADAR] Escaneando: MIAMI (LMD)...")

    try:
        if page.url.startswith("https://www.citaconsular.es"):
            log_fn("   🔄 [RECARGA VIVA] Actualizando disponibilidad del Consulado de Miami...")
            page.reload(wait_until="domcontentloaded")
        else:
            page.goto(URL_MIAMI_LMD, wait_until="domcontentloaded")

        time.sleep(random.uniform(2.5, 3.5))

        # Paso 1: Superar Cloudflare Turnstile
        if not _superar_cloudflare(page, log_fn):
            log_fn("   [AVISO] Miami bloqueado por Cloudflare.")
            return False, "", {}
        ...
        if num_slots > 0 and not sin_citas_miami:
            fecha_encabezado = ""
            ...
            log_fn(f"   [ALERTA CRITICA] CITAS LIBRES EN MIAMI LMD ({len(todos_dias_verdes)} días detectados).")
            return True, resumen_general, info_slots

        if sin_citas_miami or num_slots == 0:
            log_fn("   [i] Miami LMD: Sin horas disponibles en este momento (Agenda completa).")
            return False, "", {}

        return False, "", {}

    except Exception as e:
        log_fn(f"   [!] Excepción al escanear Miami: {e}")
        return False, "", {}


def inyectar_cita_miami(page, slot_info, candidato, log_fn=print):
    """Inyecta automáticamente la selección de hora y las credenciales del cliente en Miami (#signupsecondappointment)."""
    try:
        ...
        time.sleep(1.0)
        log_fn(f"   ✅ [LISTO] Credenciales inyectadas en pantalla para {candidato.get('nombreCompleto')} (Miami).")
        return True

    except Exception as e:
        log_fn(f"   [!] Error al inyectar credenciales en Miami: {e}")
        return False
