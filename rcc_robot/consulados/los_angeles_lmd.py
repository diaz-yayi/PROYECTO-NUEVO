...
"""
=============================================================================
CONTROLADOR ESPECIALIZADO: LOS ÁNGELES LMD (v4.0 ANTI-INTERRUPCIÓN DOM)
=============================================================================
"""
...
URL_LOS_ANGELES_LMD = "https://www.citaconsular.es/es/hosteds/widgetdefault/2244a1a71808fe950abb503cd46c2e517"
...
def _seleccionar_tramite_lmd_la(page, log_fn):
    """Paso 3: Hace clic exacto en el trámite 'PRESENTACIÓN DOCUMENTACIÓN LEY MEMORIA DEMOCRÁTICA -LMD' (bkt1396274)."""
    try:
        for ciclo in range(4):
            time.sleep(1.0)
            try:
                if "#datetime" in page.url or "#agendas" in page.url or "no hay agendas" in page.locator("body").inner_text().lower():
                    return True
            except Exception:
                pass

            servicio_btn = page.locator("a[href*='bkt1396274'], a:has-text('PRESENTACIÓN DOCUMENTACIÓN'), [class*='ServiceDataContainer']:has-text('MEMORIA DEMOCRÁTICA')")
            if servicio_btn.count() > 0 and servicio_btn.first.is_visible():
                log_fn("   [PASO 3] Pulsando en el trámite: 'PRESENTACIÓN DOCUMENTACIÓN LMD'...")
                box = servicio_btn.first.bounding_box()
                if box and box["width"] > 0:
                    mover_raton_humano_y_clic(page, box["x"] + box["width"] / 2, box["y"] + box["height"] / 2)
                try:
                    servicio_btn.first.click(force=True)
                except Exception:
                    pass
                time.sleep(3.0)
            else:
                try:
                    page.evaluate("window.location.hash = '#selectservice/bkt1396274';")
                except Exception:
                    pass
                time.sleep(2.5)
...
def verificar_citas_los_angeles(page, config, log_fn=print):
    """
    Función principal de escaneo para Los Ángeles LMD.
    Retorna: (hay_citas: bool, resumen_general: str, info_slots: dict)
    """
    log_fn("\n[RADAR] Escaneando: LOS ÁNGELES (LMD)...")

    try:
        if page.url.startswith("https://www.citaconsular.es"):
            log_fn("   🔄 [RECARGA VIVA] Actualizando disponibilidad del Consulado de Los Ángeles...")
            page.reload(wait_until="domcontentloaded")
        else:
            page.goto(URL_LOS_ANGELES_LMD, wait_until="domcontentloaded")

        time.sleep(random.uniform(2.5, 3.5))

        # Paso 1: Superar Cloudflare Turnstile
        if not _superar_cloudflare(page, log_fn):
            log_fn("   [AVISO] Los Ángeles bloqueado por Cloudflare.")
            return False, "", {}
        ...
        # Paso 3: Seleccionar Trámite LMD Exacto (bkt1396274)
        _seleccionar_tramite_lmd_la(page, log_fn)
        ...
        sin_citas_la = (
            "no hay agendas disponibles" in texto_lower or
        ...
        if num_slots > 0 and not sin_citas_la:
            fecha_encabezado = ""
            ...
            log_fn(f"   [ALERTA CRITICA] CITAS LIBRES EN LOS ÁNGELES LMD ({len(todos_dias_verdes)} días detectados).")
            return True, resumen_general, info_slots

        if sin_citas_la or num_slots == 0:
            log_fn("   [i] Los Ángeles LMD: Sin horas disponibles en este momento (No hay agendas disponibles).")
            return False, "", {}

        return False, "", {}

    except Exception as e:
        log_fn(f"   [!] Excepción al escanear Los Ángeles: {e}")
        return False, "", {}


def inyectar_cita_los_angeles(page, slot_info, candidato, log_fn=print):
    """Inyecta automáticamente la selección de hora y las credenciales del cliente en Los Ángeles (#signupsecondappointment)."""
    try:
        ...
        campo_usr = page.locator("#idIptBktSignInlogin, input[name='login'], input[placeholder*='ID']")
        ...
        time.sleep(1.0)
        log_fn(f"   ✅ [LISTO] Credenciales inyectadas en pantalla para {candidato.get('nombreCompleto')} (Los Ángeles).")
        return True

    except Exception as e:
        log_fn(f"   [!] Error al inyectar credenciales en Los Ángeles: {e}")
        return False
