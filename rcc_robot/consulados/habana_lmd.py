...
"""
=============================================================================
CONTROLADOR ESPECIALIZADO: LA HABANA LMD (v6.0 RADIOGRAFÍA ESTRUCTURADA POR MESES)
=============================================================================
Flujo:
  1. Recarga en vivo (Live Reload) de datos frescos del Consulado.
  2. Puerta Cloudflare Turnstile con trayectoria humana.
  3. Botón verde [ Continue / Continuar ].
  4. Modal de condiciones (#dialog-confirm) -> Clic en [ ACEPTAR ].
  5. Selección del trámite 'PRESENTACIÓN DE DOCUMENTACIÓN LMD' (#selectservice/bkt897578).
  6. Pantalla de Calendario (#datetime):
     - Apertura física del Datepicker.
     - Extracción y agrupación jerárquica de TODOS los días habilitados por MESES.
     - Extracción de franjas horarias.
  7. Inyección asistida de credenciales y navegación al día específico.
=============================================================================
"""
...
URL_HABANA_LMD = "https://www.citaconsular.es/es/hosteds/widgetdefault/28330379fc95acafd31ee9e8938c278ff"
...
def _superar_pantalla_continuar(page, log_fn):
    """Paso 2: Detecta y hace clic en el botón verde [ Continue / Continuar ] hasta que desaparezca."""
    try:
...
def _superar_modal_condiciones_habana(page, log_fn):
    """Paso 3: Cierra el diálogo modal de condiciones de La Habana haciendo scroll y pulsando [ ACEPTAR ]."""
    try:
        for _ in range(4):
            time.sleep(1.0)
            dialogo = page.locator("#dialog-confirm, .ui-dialog-content")
            if dialogo.count() > 0 and dialogo.first.is_visible():
                log_fn("   [PASO 3] Modal de condiciones de La Habana detectado. Pulsando [ ACEPTAR ]...")
                page.evaluate("""() => {
                    const dialog = document.getElementById('dialog-confirm') || document.querySelector('.ui-dialog-content');
                    if (dialog) dialog.scrollTop = dialog.scrollHeight;
                    const all = Array.from(document.querySelectorAll('div, button, a, span'));
                    const btn = all.find(el => el.innerText && el.innerText.toUpperCase().includes('ACEPTAR') && el.children.length === 0 && el.offsetParent !== null);
                    if (btn) {
                        btn.click();
        ...
        log_fn(f"   [CONDICIONES] Nota: {e}")
        return False


def _seleccionar_tramite_lmd(page, log_fn):
    """Paso 4: Hace clic en el trámite 'PRESENTACIÓN DE DOCUMENTACIÓN LEY MEMORIA DEMOCRÁTICA'."""
    try:
        for ciclo in range(4):
            time.sleep(1.0)
            if "#datetime" in page.url or "cambiar de día" in page.locator("body").inner_text().lower():
                return True

            servicio_btn = page.locator("a[href*='bkt897578'], a[href*='selectservice'], .clsBktServiceDataContainer")
            if servicio_btn.count() > 0 and servicio_btn.first.is_visible():
                log_fn("   [PASO 4] Seleccionando trámite: 'PRESENTACIÓN DE DOCUMENTACIÓN LMD'...")
                page.evaluate("""() => {
                    const s = document.querySelector("a[href*='bkt897578']") || document.querySelector(".clsBktServiceDataContainer") || document.querySelector("a[href*='selectservice']");
                    if (s) s.click();
                }""")
                time.sleep(3.0)
            else:
...
def _extraer_radiografia_por_meses(page):
    ...
    Retorna: diccionario estructurado {"SEPTIEMBRE 2026": [16, 17, ...], "OCTUBRE 2026": [...]}
    y la lista plana de días.
    """
    mapa_meses = {}
    dias_totales = []
...
def verificar_citas_habana(page, config, log_fn=print):
    """
    Función principal de escaneo para La Habana LMD con Radiografía Jerárquica por Meses.
    Retorna: (hay_citas: bool, resumen_general: str, info_slots: dict)
    """
    log_fn("\n[RADAR] Escaneando: LA HABANA (LMD)...")

    try:
        if page.url.startswith("https://www.citaconsular.es"):
            log_fn("   🔄 [RECARGA VIVA] Actualizando disponibilidad del Consulado de La Habana...")
            page.reload(wait_until="domcontentloaded")
        else:
            page.goto(URL_HABANA_LMD, wait_until="domcontentloaded")

        time.sleep(random.uniform(2.5, 3.5))

        # Paso 1: Superar Cloudflare Turnstile
        if not _superar_cloudflare(page, log_fn):
            log_fn("   [AVISO] La Habana bloqueado por Cloudflare.")
            return False, "", {}
        ...
        # Paso 3: Superar Modal de Condiciones [ ACEPTAR ]
        _superar_modal_condiciones_habana(page, log_fn)

        # Paso 4: Seleccionar Trámite LMD
        _seleccionar_tramite_lmd(page, log_fn)

        time.sleep(random.uniform(2.5, 3.5))
        texto_visible = page.locator("body").inner_text()
        texto_lower = texto_visible.lower()

        # Paso 5: Evaluar Disponibilidad Real
        sin_citas_habana = (
        ...
        if num_slots > 0 and not sin_citas_habana:
            # 1. Extraer fecha visible del encabezado
            fecha_encabezado = ""
            ...
            # 2. Radiografía Completa por Meses
            mapa_meses, todos_dias_verdes = _extraer_radiografia_por_meses(page)

            # Rango horario de la vista inmediata
            ...
            log_fn(f"   [ALERTA CRITICA] CITAS LIBRES EN LA HABANA LMD ({len(todos_dias_verdes)} días detectados).")
            return True, resumen_general, info_slots

        if sin_citas_habana or num_slots == 0:
            log_fn("   [i] La Habana LMD: Sin horas disponibles en este momento (Agenda completa).")
            return False, "", {}

        return False, "", {}

    except Exception as e:
        log_fn(f"   [!] Excepción al escanear La Habana: {e}")
        return False, "", {}


def inyectar_cita_habana(page, slot_info, candidato, log_fn=print):
    """Inyecta automáticamente la selección de hora y las credenciales del cliente en La Habana."""
    try:
        ...
        log_fn(f"   🔑 Inyectando credenciales de {candidato.get('nombreCompleto')} (Usuario: {usuario})...")

        campo_usr = page.locator("input[type='text'], input[name*='user'], input[id*='user'], input[placeholder*='usuario'], input[placeholder*='login'], #username, #user")
        if campo_usr.count() > 0:
            campo_usr.first.fill(usuario)

        campo_pwd = page.locator("input[type='password'], input[name*='pass'], input[id*='pass'], #password, #pass")
        if campo_pwd.count() > 0:
            campo_pwd.first.fill(password)

        time.sleep(1.0)
        log_fn(f"   ✅ [LISTO] Credenciales inyectadas en pantalla para {candidato.get('nombreCompleto')}.")
        return True

    except Exception as e:
        log_fn(f"   [!] Error al inyectar credenciales: {e}")
        return False
